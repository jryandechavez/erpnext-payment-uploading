...
The page checks live Sales Invoice outstanding balances and creates one draft Journal Entry for the complete uploaded batch only after review. Users can add multiple configured debit rows. Each invoice is credited to its own receivable account and exact `Sales Invoice.customer`, with its Sales Invoice reference and due date. Write-off is calculated per invoice, so every non-zero invoice difference creates a separate row tagged through the `Write-off Customer` and `Write-off Sales Invoice` Link fields; its User Remark also identifies the invoice, customer, and due date. Formal Sales Invoice references remain on receivable rows because ERPNext rejects them on a different write-off account. EWT is the final debit line. The completed draft opens in ERPNext for final review and submission.
